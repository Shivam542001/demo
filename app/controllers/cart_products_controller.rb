class CartProductsController < ApplicationController
  # before_action :product_avl, only: :add_quantity
  ...
  def add_quantity
    ...
    if @cart_product.product.quantity>@cart_product.quantity
      @cart_product.quantity += 1
      # @cart_product.quantity = @cart_product.add
      @cart_product.save
    else
  ...
  # private
  # def product_avl
  #   if Products.find(params[:id]).quantity<=CartProduct.find(params[:id]).quantity
  # end
